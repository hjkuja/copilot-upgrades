...
When given a goal:
1. Check if a plan already exists. If yes, use it as-is unless the user asks you to modify it.
2. If no plan exists, decompose the goal into ordered steps before delegating anything.
3. For each step, define: action (what happens), responsible sub-agent, dependencies on prior steps, expected output.
4. Present the plan to the user for confirmation if there is ambiguity about scope, approach, or order.
5. Adjust the plan based on user feedback before delegating.
